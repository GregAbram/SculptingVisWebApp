...
# blender --background --python Automating_lod_mapping.py -- "D:/path/to/input/obj" "D:/path/to/output" 10000 1000 100
# - No slash at end of input files
# - The three numbers at the end specify that three levels-of-detail will be created, with 10000, 1000, & 100 faces each, respectively. 
#   If no numbers are given, no LODS will be created, and any number of faceCounts can be specified in any order.
...
if len(inputFile.rsplit('/', 1)) == 2:
  outputDir = inputFile.rsplit('/', 1)[0]
else:
  outputDir = '.'

os.mkdir(os.path.join(outputDir,  'normalmaps'))
...
# Make a directory
	...
	# Create other lods
	for i in range(1, len(target_face_counts) + 1):
		...
		# Decimate the mesh
		starting_faces = len(lods[i].data.polygons)
		goal_faces = target_face_counts[i-1]
		ratio = min(1,goal_faces / starting_faces)
...
# Disable all LODS before exporting each
for i in range(0,len(lods)):
	lods[i].select = False

# export each LOD individually
for i in range(0,len(lods)):
	if i > 0:
		lods[i].select = True
		bpy.ops.export_scene.obj(filepath=os.path.join(outputDir, 'meshes/LOD' + str(i) + '.obj'),
